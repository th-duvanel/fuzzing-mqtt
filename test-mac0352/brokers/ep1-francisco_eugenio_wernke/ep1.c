        ...
        if ( (childpid = fork()) == 0) {
            ...
            while((n = read(connfd, recvline, MAXLINE)) > 0) {
                ...
                case CONNECT:
                    ...
                    break;
                case PUBLISH:
                    topicNamePub = retrieveTopicFromPublish(recvline);
                    message = retrieveMessage(recvline);
                    publishToTopic(topicNamePub, message);
                    break;
                case SUBSCRIBE:
                    topicNameSub = retrieveTopicFromSubscribe(recvline);
                    topicPath = createTopic(topicNameSub);
                    sendSuback(connfd);
