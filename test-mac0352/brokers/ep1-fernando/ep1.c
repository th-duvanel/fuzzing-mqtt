/*  EP1 de MAC0352
    ...
    desenvolvido a partir do código do 
...
/* Biblioteca de funções para EP1 */
#include "functions.h"
...
int main (int argc, char **argv) {
    ...
    pid_t childpid;
    /* Armazena o primeiro byte do pacote recebido do cliente */
    unsigned char ptype;
	...
	for (;;) {
        ...
        if ( (childpid = fork()) == 0) {
            ...
            while((read(connfd, &ptype, 1)) > 0) {
                packetParser(ptype, connfd);
                if (ptype == 0xe0) break; 
            }
